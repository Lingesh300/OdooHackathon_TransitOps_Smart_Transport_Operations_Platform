...
const NAV_ITEMS = [
  ...
  { route: "fuel-expenses",   label: "Fuel & Expenses",   module: "fuel" },
...
// ---------------- Router ----------------
// URL scheme: #/login, #/dashboard, #/fleet, #/drivers, #/trips,
...
const SCREEN_MAP = {
  ...
  "fuel-expenses": Screens.fuelExpenses,
...
function navigate(route) {
  ...
  document.getElementById("page-content").innerHTML = (SCREEN_MAP[route] || Screens.dashboard)();
}
...
// Support deep-linking / back-forward / manual URL edits, e.g. opening
// straight to #/fleet after login, or #/login to force sign-out view.
window.addEventListener("hashchange", () => {
  const route = window.location.hash.replace(/^#\/?/, "");
  if (!DB.currentUser) { showLogin(); return; }
  if (route && SCREEN_MAP[route] && route !== getCurrentRoute()) {
    navigate(route);
  }
});
function getCurrentRoute() {
  const active = document.querySelector("#sidebar-nav a.active");
  return active ? active.dataset.route : null;
}
...
const Actions = {
  ...
  openEditVehicle(id) {
    ...
    UI.modal("Edit Vehicle — " + v.name, `
      <div id="modal-error"></div>
      ${UI.field("Registration No.", `<input id="mv-regno" value="${UI.escape(v.regNo)}" />`)}
      ${UI.field("Name/Model", `<input id="mv-name" value="${UI.escape(v.name)}" />`)}
      <div class="two-col">
        ${UI.field("Type", `<select id="mv-type">${["Van","Truck","Mini"].map(t=>`<option ${t===v.type?"selected":""}>${t}</option>`).join("")}</select>`)}
        ${UI.field("Capacity (kg)", `<input id="mv-capacity" type="number" value="${v.capacity}" />`)}
      </div>
      ${UI.field("Odometer (km)", `<input id="mv-odometer" type="number" value="${v.odometer}" />`)}
      ${UI.field("Acquisition Cost", `<input id="mv-cost" type="number" value="${v.acqCost}" />`)}
      ${UI.field("Status", `<select id="mv-status">${["Available","On Trip","In Shop","Retired"].map(s=>`<option ${s===v.status?"selected":""}>${s}</option>`).join("")}</select>`)}
      <div class="form-actions">
        <button class="btn btn-primary" onclick="Actions.submitEditVehicle('${id}')">Save Changes</button>
        <button class="btn btn-ghost" onclick="UI.closeModal()">Cancel</button>
      </div>
    `);
  },
  submitEditVehicle(id) {
    try {
      DB.actions.updateVehicle(id, {
        regNo: document.getElementById("mv-regno").value.trim(),
        name: document.getElementById("mv-name").value.trim(),
        type: document.getElementById("mv-type").value,
        capacity: Number(document.getElementById("mv-capacity").value),
        odometer: Number(document.getElementById("mv-odometer").value),
        acqCost: Number(document.getElementById("mv-cost").value),
  ...
  openEditDriver(id) {
    const d = DB.get("drivers", id);
    UI.modal("Edit Driver — " + d.name, `
      <div id="modal-error"></div>
      ${UI.field("Name", `<input id="md-name" value="${UI.escape(d.name)}" />`)}
      ${UI.field("License No.", `<input id="md-license" value="${UI.escape(d.licenseNo)}" />`)}
      <div class="two-col">
        ${UI.field("Category", `<select id="md-category">${["LMV","HMV"].map(c=>`<option ${c===d.category?"selected":""}>${c}</option>`).join("")}</select>`)}
        ${UI.field("License Expiry", `<input id="md-expiry" type="date" value="${d.expiry}" />`)}
      </div>
      ${UI.field("Contact Number", `<input id="md-contact" value="${UI.escape(d.contact)}" />`)}
      ${UI.field("Status", `<select id="md-status">${["Available","On Trip","Off Duty","Suspended"].map(s=>`<option ${s===d.status?"selected":""}>${s}</option>`).join("")}</select>`)}
      <div class="form-actions">
        <button class="btn btn-primary" onclick="Actions.submitEditDriver('${id}')">Save Changes</button>
        <button class="btn btn-ghost" onclick="UI.closeModal()">Cancel</button>
      </div>
    `);
  },
