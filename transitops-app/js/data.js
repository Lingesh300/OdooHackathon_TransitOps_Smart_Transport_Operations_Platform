...
const DB = {
  ...
  actions: {
    ...
    updateVehicle(id, data) {
      if (data.regNo && DB.vehicles.some(v => v.id !== id && v.regNo.toLowerCase() === data.regNo.toLowerCase())) {
        throw new Error("Registration number must be unique.");
      }
    ...
    deleteVehicle(id) {
      const v = DB.get("vehicles", id);
      if (v && v.status === "On Trip") throw new Error("Cannot delete a vehicle that is currently On Trip.");
      DB.vehicles = DB.vehicles.filter(x => x.id !== id);
    },

    updateDriver(id, data) {
      if (data.licenseNo && DB.drivers.some(d => d.id !== id && d.licenseNo.toLowerCase() === data.licenseNo.toLowerCase())) {
        throw new Error("License number must be unique.");
      }
    ...
    deleteDriver(id) {
      const d = DB.get("drivers", id);
      if (d && d.status === "On Trip") throw new Error("Cannot delete a driver that is currently On Trip.");
